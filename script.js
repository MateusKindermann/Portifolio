...
const dataInicio = [
                    (new Date(2023, 4, 1)).getTime(), // 0
                    (new Date(2023, 4, 1)).getTime(), // 1
                    (new Date(2023, 6, 1)).getTime(), // 2
                    (new Date(2023, 7, 1)).getTime(), // 3
                    (new Date(2023, 7, 1)).getTime() // 4
                    ]
...
skills.forEach(  (elemento, index) => {
    ...
    elemento.addEventListener('mouseover', (evento) => {
        ...
        descricao.innerHTML = `<p>${sobreSkill[index1]} ${tempoDeExperiencia} ${tempo} </p>` ;
    } )
